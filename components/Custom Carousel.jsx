          ...
          {carouselImages.map((carouselImage, index) => (
            <CarouselItem key={index}>
              <div className="h-full relative bg-no-repeat bg-cover" style={{ backgroundImage: `url("${carouselImage}")` }}>
                <div style={{ backgroundColor: "rgba(0,0,0,0.35)" }} className="h-full w-full text-white flex items-center justify-center flex-col">
                  <h1 className="font-semibold text-2xl md:text-5xl lg:text-7xl mb-4">{services[index].name}</h1>
                  <p className="mx-8 sm:mx-0 md:max-w-5xl md:text-justify text-sm">{services[index].description}</p>
                </div>
              </div>
            </CarouselItem>
          ))}
